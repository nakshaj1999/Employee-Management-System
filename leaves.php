    ...
    <nav>
    	<nav class="navbar navbar-expand-sm fixed-top navbar-dark ">        <!-- BAR COLOR SCHEME AND STARTING -->
          ...
          <table cellpadding="15" class="login">
            <tr>
              <td colspan="2" align="center" style="background-color: rgb(0,0,0); background-color: rgba(10,10,100,0.6);color: white;">LEAVE STATUS</td>
            </tr>
             ...
             <?php
                  ...
                  if($sql){
                    ...
                    while ($row=$result-> fetch_assoc()) {
                      if($sql){
                        echo "<tr><th>EMAIL :</th><td>".$row['email']."</td></tr><tr><th>NAME :</th><td>".$row['name']."</td></tr><tr><th>TYPE :</th><td>".$row['ltype']."</td></tr><tr><th>FROM :</th><td>".$row['dfrom']."    ".$row['tfrom']."</td></tr><tr><th>TO :</th><td>".$row['dto']." ".$row['tto']."</td></tr><tr><th>REASON :</th><td>".$row['reason']."</td></tr><tr><th>STATUS :</th><td>".$row['status']."</td></tr><tr><td colspan='2' align='center'><form action='' method='post'><input type='hidden' name='lno' value=" .$row['lno'] ."><input type='submit' class='btn btn-lg btn-primary' name='submit' value='Cancel' style='color: white;'></form></td></tr>";
